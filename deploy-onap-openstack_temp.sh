...
VM_DISK_MAX=40             # GB - max VM disk size
VM_MEM_MAX=16760           # MB - max VM RAM size
VM_CPUS_MAX=4              # max count of VM CPUs
...
for SUBNET in public-subnet private-subnet onap_private_subnet ; do
    SUBNET_CIDR=`openstack subnet list --name $SUBNET -f value -c Subnet`
        openstack security group rule create --remote-ip 0.0.0.0/0 --proto tcp \
